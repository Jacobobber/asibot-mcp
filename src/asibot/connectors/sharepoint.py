...
from asibot.connectors.base import Connector, Document
from asibot.rag.ingest import ingest_text
...
class SharePointConnector(Connector):
    ...
    def register_tools(self, mcp: FastMCP):
        ...
        async def sharepoint_search(query: str, ctx: Context, limit: int = 10) -> str:
            ...
            if err:
                return err
            r = await client.post(f"{GRAPH}/search/query", json={
                "requests": [{"entityTypes": ["driveItem"], "query": {"queryString": query}, "from": 0, "size": limit}]
            })
            r.raise_for_status()
        ...
        async def sharepoint_list_files(ctx: Context, folder_path: str = "", site: str = "") -> str:
            ...
            url = f"{GRAPH}/sites/{sid}/drive/root:/{folder_path}:/children" if folder_path else f"{GRAPH}/sites/{sid}/drive/root/children"
            r = await client.get(url)
            r.raise_for_status()
        ...
        async def sharepoint_read_file(file_path: str, ctx: Context, site: str = "") -> str:
            ...
            if not sid:
                return "No site configured."
            r = await client.get(f"{GRAPH}/sites/{sid}/drive/root:/{file_path}")
            r.raise_for_status()
            item = r.json()
        ...
        async def sharepoint_list_sites(ctx: Context, query: str = "") -> str:
            ...
            if err:
                return err
            r = await client.get(f"{GRAPH}/sites", params={"search": query or "*"})
            r.raise_for_status()
